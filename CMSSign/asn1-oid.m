...
asn1_oid(const asn1_token_t *token, asn1_oid_t *oid) {
	...
	*oid = (asn1_oid_t){0};
	...
	if (token->data == NULL || token->length == 0) {
		return ERROR(ASININE_ERR_MALFORMED, ASLOG("OID: zero length"));
	}
	...
	if ((*(token->data + token->length - 1) & OID_CONTINUATION_MASK) != 0) {
		return ERROR(ASININE_ERR_MALFORMED, ASLOG("OID: no end marker"));
	}
	...
	for (data = token->data; data < token->data + token->length; data++) {
		if (arc == 0 && *data == 0x80) {
			...
			// value 0x80"
			return ERROR(ASININE_ERR_MALFORMED, ASLOG("OID: leading byte is zero"));
		}
		...
		if (arc_bits > sizeof(arc) * 8) {
			return ERROR(ASININE_ERR_MEMORY, ASLOG("OID: arc too long"));
		}
		...
		if ((*data & OID_CONTINUATION_MASK) == 0) {
			if (is_first_arc) {
				...
				// one.
				asn1_oid_arc_t x = MINI(arc, 80) / 40;

				if (!append_arc(oid, x)) {
					return ERROR(ASININE_ERR_MEMORY, ASLOG("OID: too many arcs"));
				}
