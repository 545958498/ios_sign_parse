...
x509_find_issuer(
	...
	return ERROR(ASININE_ERR_NOT_FOUND, ASLOG("issuer: no match in trust store"));
}
...
process_certificate(x509_path_t *path, const x509_cert_t *cert) {
	...
	if (!signature_is_compatible(
		...
		return ERROR(ASININE_ERR_INVALID,
		    ASLOG("signature: algorithm doesn't match public key"));
	}
	...
	if (!x509_name_eq(&cert->issuer, &path->issuer_name, NULL)) {
		return ERROR(ASININE_ERR_INVALID, ASLOG("issuer: no match"));
	}
...
x509_path_add(x509_path_t *path, const x509_cert_t *cert) {
	...
	if (cert->key_usage != 0 &&
		...
		return ERROR(
		    ASININE_ERR_INVALID, ASLOG("certificate: missing signing key usage"));
	}
