...
//    printf("parse cert\n");
	...
	if (asn1_is(token, ASN1_CLASS_CONTEXT, 0, ASN1_ENCODING_CONSTRUCTED)) {
		...
		if (!asn1_is_int(token)) {
			return ERROR(ASININE_ERR_INVALID, ASLOG("cert: is not int"));
		}
	...
	if (!asn1_is_int(token)) {
		return ERROR(ASININE_ERR_INVALID, ASLOG("cert: serialNumber, is not int"));
	}
	...
	if (cert->signature.algorithm != sig_check.algorithm) {
		...
		return ERROR(
		    ASININE_ERR_INVALID, ASLOG("cert: signature algorithm doesn't match"));
	}

	// signature
	NEXT_TOKEN(parser);
	if (!asn1_is_bitstring(token)) {
		return ERROR(ASININE_ERR_INVALID, ASLOG("cert: signature is not bitString"));
	}
	...
	if (token->length < 1 || token->data[0] != 0) {
		return ERROR(
		    ASININE_ERR_MALFORMED, ASLOG("cert: signature has invalid prefix"));
	}
	...
	if (cert->is_ca && cert->subject.num == 0) {
		return ERROR(ASININE_ERR_INVALID, ASLOG("cert: missing subject name (as CA)"));
	}

	if ((cert->key_usage & X509_KEYUSE_CRL_SIGN) != 0 &&
	    cert->subject.num == 0) {
		return ERROR(
		    ASININE_ERR_INVALID, ASLOG("cert: missing subject name (to sign)"));
	}
...
parse_optional(asn1_parser_t *parser, x509_cert_t *cert) {
	...
	if (cert->version >= X509_V2) {
		...
		if (cert->version != X509_V3) {
			...
			return ERROR(
			    ASININE_ERR_INVALID, ASLOG("cert: extensions should not be present"));
		}

		if (!asn1_is(token, ASN1_CLASS_CONTEXT, 3, ASN1_ENCODING_CONSTRUCTED)) {
			return ERROR(ASININE_ERR_INVALID, ASLOG("optional: error"));
		}
...
parse_extensions(asn1_parser_t *parser, x509_cert_t *cert) {
	...
	while (!asn1_eof(parser)) {
		...
		if (!asn1_is_oid(token)) {
			return ERROR(ASININE_ERR_INVALID, ASLOG("extension: not a oid"));
		}
		...
		if (!asn1_is_octetstring(token)) {
			return ERROR(ASININE_ERR_INVALID, ASLOG("extension: not octetstring"));
		}
		...
		if (extn_parser != NULL) {
            ...
            char reason[128];
            char oid[32];
            asn1_oid_to_string(oid, 32, &id);
            sprintf(reason, ASPLOG("unknown critical extension: ", "%s"), oid);
			return ERROR(ASININE_ERR_UNSUPPORTED, reason);
		}
...
const signature_lookup_t *
find_signature_algorithm(const asn1_oid_t *oid) {
	size_t i;
	for (i = 0; i < NUMS(signature_algorithms); i++) {
...
parse_signature_algo(asn1_parser_t *parser, x509_signature_t *signature) {
	...
	NEXT_TOKEN(parser);
	if (!asn1_is_oid(token)) {
		return ERROR(ASININE_ERR_INVALID, ASLOG("signature_algo: not a oid"));
	}
	...
	if (result == NULL) {
		return ERROR(ASININE_ERR_UNSUPPORTED, ASLOG("signature: unknown algorithm"));
	}
...
parse_validity(asn1_parser_t *parser, x509_cert_t *cert) {
	...
	if (!asn1_is_time(token)) {
		return ERROR(ASININE_ERR_INVALID, ASLOG("validity: not a time"));
	}
...
_x509_parse_null_or_empty_args(asn1_parser_t *parser) {
	...
	if (!asn1_is_null(token)) {
		return ERROR(ASININE_ERR_INVALID, ASLOG("null_or_empty_args: not null"));
	}
...
parse_empty_args(asn1_parser_t *parser, x509_signature_t *sig) {
	...
	if (!asn1_eof(parser)) {
		return ERROR(ASININE_ERR_MALFORMED, ASLOG("algorithm: non-empty args"));
	}
...
parse_extn_key_usage(asn1_parser_t *parser, x509_cert_t *cert) {
	...
	if (!asn1_is_bitstring(&parser->token)) {
		return ERROR(ASININE_ERR_INVALID, ASLOG("extn_key_usage: not bitstring"));
	}
	...
	if ((cert->key_usage & X509_KEYUSE_DECIPHER_ONLY) != 0 &&
		...
		return ERROR(ASININE_ERR_INVALID,
		    ASLOG("key usage: both decipher only and encipher only asserted"));
	}
	 ...
	 */
	if (cert->key_usage == 0) {
		return ERROR(ASININE_ERR_INVALID, ASLOG("key usage: no usage asserted"));
	}
...
parse_extn_ext_key_usage(asn1_parser_t *parser, x509_cert_t *cert) {
	...
	while (!asn1_eof(parser)) {
		...
		if (!asn1_is_oid(&parser->token)) {
			return ERROR(ASININE_ERR_INVALID, ASLOG("extn_ext_key_usage: not a oid"));
		}
...
parse_extn_developerSign(asn1_parser_t *parser, x509_cert_t *cert) {
    ...
    if (!asn1_is_null(&parser->token)) {
        return ERROR(ASININE_ERR_INVALID, ASLOG("developerSign: not null"));
    }
...
parse_extn_basic_constraints(asn1_parser_t *parser, x509_cert_t *cert) {
	...
	if (value < 0) {
		return ERROR(
		    ASININE_ERR_INVALID, ASLOG("basic constraints: negative path length"));
	}

	if (value > INT8_MAX) {
		return ERROR(
		    ASININE_ERR_UNSUPPORTED, ASLOG("basic constraints: path length too high"));
	}
