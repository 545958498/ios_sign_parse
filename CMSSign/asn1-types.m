...
validate_string(const asn1_token_t *token) {
	...
	if (token->type.cls != ASN1_CLASS_UNIVERSAL) {
		return ERROR(ASININE_ERR_MALFORMED, ASLOG("string: not of class universal"));
	}
	...
	case ASN1_TAG_PRINTABLESTRING:
		for (data = token->data; data < data_end; data++) {
			...
			if (*data < 0x27 || *data > 0x7a) {
				return ERROR(
				    ASININE_ERR_MALFORMED,ASLOG( "string: invalid character"));
			}
		...
		for (data = token->data; data < data_end; data++) {
			...
			case LEADING:
				...
				if (0xC2 <= byte && byte < 0xD0) {
					...
					return ERROR(ASININE_ERR_MALFORMED,
					    ASLOG("utf8string: invalid leading character"));
				}
			...
			case CONTINUATION:
				if (byte < 0x80 || byte >= 0xC0) {
					return ERROR(ASININE_ERR_MALFORMED,
					    ASLOG("utf8string: invalid continuation"));
				}
	...
	default:
		return ERROR(ASININE_ERR_INVALID, ASLOG("string: unknown tag"));
	}
...
asn1_string(const asn1_token_t *token, char *buf, size_t len) {
	...
	if (len <= token->length) {
		return ERROR(ASININE_ERR_MEMORY, ASLOG("string: buffer too small"));
	}
	...
	if (strlen(buf) != token->length) {
		return ERROR(ASININE_ERR_INVALID, ASLOG("string: contains NUL"));
	}
...
asn1_bitstring(const asn1_token_t *token, uint8_t *buf, const size_t len) {
	...
	// 8.6.2.2
	if (token->length == 0) {
		return ERROR(ASININE_ERR_MALFORMED, ASLOG("bitstring: zero length"));
	}

	if (buf != NULL && token->length - 1 > len) {
		return ERROR(ASININE_ERR_MEMORY, ASLOG("bitstring: buffer too small"));
	}
	...
	if (unused_bits > 7) {
		return ERROR(ASININE_ERR_MALFORMED, ASLOG("bitstring: too many unused bits"));
	}
	...
	if (token->length == 1) {
		if (unused_bits != 0) {
			return ERROR(
			    ASININE_ERR_MALFORMED, ASLOG("bitstring: invalid zero value"));
		}
	...
	if (token->data[token->length - 1] == 0) {
		return ERROR(ASININE_ERR_MALFORMED, ASLOG("bitstring: invalid padding"));
	}
	...
	if (unused_bits > 0) {
		...
		if ((token->data[token->length - 1] & unused_bits) != 0) {
			return ERROR(
			    ASININE_ERR_MALFORMED, ASLOG("bistring: unused bits are set"));
		}
	}
...
asn1_int(const asn1_token_t *token, asn1_word_t *value) {
	...
	if (token->length == 0) {
		return ERROR(ASININE_ERR_INVALID, ASLOG("int: zero length"));
	}
	...
	if (token->length > 1) {
		...
		if (leading == 0 || leading == 0x1ff) {
			return ERROR(ASININE_ERR_MALFORMED, ASLOG("int: not smallest encoding"));
		}
	}
	...
	if (token->length > sizeof *value) {
		return ERROR(ASININE_ERR_MEMORY, ASLOG("int: too large"));
	}
...
asn1_uint_buf(const asn1_token_t *token, const uint8_t **buf, size_t *num) {
	...
	if ((token->data[0] & 0x80) != 0) {
		return ERROR(ASININE_ERR_INVALID, ASLOG("uint: signed integer"));
	}
...
asn1_time(const asn1_token_t *token, asn1_time_t *time) {
	...
	if (token->length < MIN_TIME_LENGTH || token->length > MAX_TIME_LENGTH) {
		return ERROR(ASININE_ERR_MALFORMED, ASLOG("time: wrong length"));
	}
	...
	for (size_t i = 0; i < token->length / 2; data += 2, i++) {
		if (!decode_pair(data, &pairs[i])) {
			return ERROR(ASININE_ERR_MALFORMED, ASLOG("time: invalid pair"));
		}
	}
	...
	if (*data != 'Z') {
		return ERROR(ASININE_ERR_MALFORMED, ASLOG("time: not in UTC"));
	}
	...
	case ASN1_TAG_UTCTIME:
		...
		if (time->year > 99) {
			return ERROR(ASININE_ERR_MALFORMED, ASLOG("time: invalid year"));
		}
	...
	default:
		return ERROR(ASININE_ERR_MALFORMED, ASLOG("time: unknown tag"));
	}
	...
	if (time->month < 1 || time->month > 12) {
		return ERROR(ASININE_ERR_MALFORMED, ASLOG("time: invalid month"));
	}

	if (time->day < 1) {
		return ERROR(ASININE_ERR_MALFORMED, ASLOG("time: invalid day"));
	...
	if (time->hour > 23) {
		return ERROR(ASININE_ERR_MALFORMED, ASLOG("time: invalid hour"));
	}

	if (time->second > 59) {
		return ERROR(ASININE_ERR_MALFORMED, ASLOG("time: invalid second"));
	}
...
asn1_bool(const asn1_token_t *token, bool *value) {
	...
	if (token->length != 1) {
		return ERROR(ASININE_ERR_MALFORMED, ASLOG("bool: wrong length"));
	}
	...
	if (data == 0x00) {
		...
		*value = true;
		...
		return ERROR(ASININE_ERR_MALFORMED, ASLOG("bool: invalid value"));
	}
...
asn1_null(const asn1_token_t *token) {
	if (token->length != 0) {
		return ERROR(ASININE_ERR_MALFORMED, ASLOG("null: length isn't zero"));
	}
...
asn1_type_to_string(char *dst, size_t num, const asn1_type_t *type) {
	int res;
	if (type->cls == ASN1_CLASS_UNIVERSAL) {
		res = snprintf(dst, num, "%s", tag_to_string(type->tag));
	} else {
		const char *class = class_to_string(type->cls);
...
type_eq(const asn1_type_t *type, asn1_class_t class, asn1_tag_t tag,
	...
	return (type->cls == class) && (type->tag == tag) &&
	       (type->encoding == encoding);
}

bool
asn1_eq(const asn1_token_t *a, const asn1_token_t *b) {
    asn_assert_nonull_bool(a);
	asn_assert_nonull_bool(b);

	return (a->length == b->length) &&
	       type_eq(&a->type, b->type.cls, b->type.tag, b->type.encoding) &&
...
asn1_is_string(const asn1_token_t *token) {
	...
	return (token->type.cls == ASN1_CLASS_UNIVERSAL) &&
