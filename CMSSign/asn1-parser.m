...
asn1_init(asn1_parser_t *parser, const uint8_t *data, size_t length) {
	asn_assert_nonull(parser);
	asn_assert_nonull(data);
...
asn1_next(asn1_parser_t *parser) {
	...
	*token       = (asn1_token_t){0};
	...
	// Type (8.1.2)
	token->type.cls    = TYPE_CLASS(*parser->current);
	...
	if (token->type.tag == TYPE_MULTIPART_TAG) {
		...
		do {
			if (!advance_pos(parser, 1)) {
				return ERROR(ASININE_ERR_MALFORMED, ASLOG("token: tag: truncated"));
			}
			...
			if (bits > ASN1_TYPE_TAG_BITS) {
				return ERROR(ASININE_ERR_MEMORY, ASLOG("token: tag: too large"));
			}
	...
	// Length (8.1.3)
	if (!advance_pos(parser, 1)) {
		return ERROR(ASININE_ERR_MALFORMED, ASLOG("token: truncated header"));
	}
	...
	if (CONTENT_LENGTH_IS_LONG_FORM(*parser->current)) {
		...
		if (num_bytes == CONTENT_LENGTH_LONG_RESERVED) {
			return ERROR(ASININE_ERR_MALFORMED, ASLOG("token: length: reserved"));
		} else if (num_bytes == 0) {   //0x80不定长编码方式, TODO
			// Indefinite form is forbidden (X.690 11/2008 8.1.3.6)
            parser->current++;
			return ERROR(ASININE_OK, ASLOG("token: length: indefinite"));
		} else if (num_bytes > sizeof token->length) {
			return ERROR(ASININE_ERR_UNSUPPORTED, ASLOG("token: length: too long"));
		}

		for (i = 0; i < num_bytes; i++) {
			if (!advance_pos(parser, 1)) {
				return ERROR(ASININE_ERR_MALFORMED, ASLOG("token: length: truncated"));
			}

			if (token->length == 0 && *parser->current == 0) {
				return ERROR(
				    ASININE_ERR_MALFORMED, ASLOG("token: lenght: leading zero"));
			}
		...
		if (token->length < CONTENT_LENGTH_LONG_MIN) {
			return ERROR(ASININE_ERR_MALFORMED,
			    ASLOG("token: length: below long-form minimum"));
		}
	} else {
	...
	if (token->length > 0) {
		...
		if (!advance_pos(parser, token->length)) {
			return ERROR(ASININE_ERR_MALFORMED, ASLOG("token: truncated content"));
		}
		token->data = data;
	}
    else{
...
asn1_abort(asn1_parser_t *parser) {
	if (parser->depth == 0) {
		return ERROR(ASININE_ERR_INVALID, ASLOG("abort: already at root"));
	}
    ...
    if(start == NULL)return ERROR(ASININE_ERR_INVALID, ASLOG("about: data = NULL"));
...
asn1_push(asn1_parser_t *parser) {
	if (parser->token.type.encoding != ASN1_ENCODING_CONSTRUCTED) {
		return ERROR(ASININE_ERR_INVALID, ASLOG("push: not constructed"));
	}
...
asn1_force_push(asn1_parser_t *parser) {
	...
	if (parser->depth + 1 > NUMS(parser->stack)) {
		return ERROR(ASININE_ERR_UNSUPPORTED, ASLOG("push: nested too deep"));
	}
...
asn1_pop(asn1_parser_t *parser) {
	if (parser->depth == 0) {
		return ERROR(ASININE_ERR_INVALID, ASLOG("pop: already at root"));
	}

	// Don't pop tokens which havent been fully parsed
	if (!asn1_eof(parser)) {
		return ERROR(ASININE_ERR_MALFORMED, ASLOG("pop: parent not fully parsed"));
	}
...
asn1_push_seq(asn1_parser_t *parser) {
	...
	if (!asn1_is_sequence(&parser->token)) {
		return ERROR(ASININE_ERR_INVALID, ASLOG("push: token not a sequence"));
	}
