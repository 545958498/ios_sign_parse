...
find_pubkey_algorithm(const asn1_oid_t *oid) {
	size_t i;
	for (i = 0; i < NUMS(pubkey_algorithms); i++) {
...
x509_parse_pubkey(asn1_parser_t *parser, x509_pubkey_t *pubkey,
	...
	*has_params = false;
	...
	if (!asn1_is_oid(&parser->token)) {
		return ERROR(ASININE_ERR_INVALID, ASLOG("pubkey: token isn't an OID"));
	}
	...
	if (result == NULL) {
		return ERROR(ASININE_ERR_UNSUPPORTED, ASLOG("pubkey: algorithm unknown"));
	}
	...
	if (!asn1_is_bitstring(&parser->token)) {
		return ERROR(ASININE_ERR_INVALID, ASLOG("pubkey: token isn't a bitstring"));
	}
...
parse_rsa_pubkey(asn1_parser_t *parser, x509_pubkey_t *pubkey) {
	...
	if (!asn1_is_int(token)) {
		return ERROR(ASININE_ERR_INVALID, ASLOG("rsa pubkey: token isn't an int"));
	}
...
find_curve(const asn1_oid_t *oid) {
	for (size_t i = 0; i < NUMS(curves); i++) {
...
parse_ecdsa_params(
	...
	if (!asn1_is_oid(&parser->token)) {
		return ERROR(ASININE_ERR_INVALID, ASLOG("ecdsa params: token isn't an OID"));
	}
	...
	if (curve == X509_ECDSA_CURVE_INVALID) {
		return ERROR(ASININE_ERR_UNSUPPORTED, ASLOG("ecsda params: unkown algorithm"));
	}
...
parse_ecdsa_pubkey(asn1_parser_t *parser, x509_pubkey_t *pubkey) {
	if (parser->token.length < 2) {
		return ERROR(ASININE_ERR_INVALID, ASLOG("ecdsa pubkey: too short"));
	}
